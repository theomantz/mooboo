...
class EditUser extends React.Component {
  ...
  checkFieldsLength() {
    const { email, location, username, description } = this.state;
    const fields = { email, location, username, description };
    ...
    const lengthMsg = (m) => {
      const l = permittedLength(m);
      return `${m} must be less than ${l} characters`;
    };
    ...
    Object.keys(fields).each((f) => {
      if (typeof f !== "string") errors.push(invalidMsg(f));
      else if (f.length > 20) errors.push(lengthMsg(f));
    });
    if (errors.length > 0) {
  ...
  handleSubmit(e) {
    ...
    if (this.checkFieldsLength()) {
      ...
      this.props
        .updateUser(formData, this.props.user)
